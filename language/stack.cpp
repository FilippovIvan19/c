...
const int LcAll = 0;
const st_t BIRD = -1245666.1313;
const st_t POISON = -66613666.1313;
...
bool StackOk(const Stack *st)
{
    return (st != NULL)
        ...
        BY_DESIGN(&& (fabs(st->data[st->capacity] - BIRD) < EPS) && (fabs(st->data[-1] - BIRD) < EPS));     ///�������� (ST->CAPACITY > 0) ������
...
int half_StackDump(const Stack *st, const char *out_file_name)
{
    ...
    fprintf(out_f, "canary#1 [-1] = %lf\n", st->data[-1]);
    fprintf(out_f, "canary#2 [%d] = %lf\n", st->capacity, st->data[st->capacity]);
...
int StackCap_x2(Stack *st)
{
    ...
    BY_DESIGN(st->data = (st_t*)realloc(st->data - 1, (st->capacity * 2 + 2) * sizeof(st_t)) + 1);  //REALLOC ���� ����� ������� NULL - fixed in StackOk
    st->capacity *= 2;
    BY_DESIGN(st->data[st->capacity] = BIRD);
