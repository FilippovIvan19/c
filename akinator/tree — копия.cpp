...
int NodeDtor(Node* node, const char* info)
{
    NODE_ASSERT(node)
...
int TreeDtor(Tree* tree, const Node* root)
{
    TREE_ASSERT(tree)
    tree != nullptr;
...
int PasteNode(Tree* tree, Node* node, Node* node_par, const int branch)
{
    ...
    if (node_par != nullptr)
        ...
        {
            case l_branch:
                if (node_par->left != nullptr)
                {
                    node->left =  node_par->left->left;
                    node->right = node_par->left->right;
                }
                node_par->left = node;
                break;
            case r_branch:
                if (node_par->right != nullptr)
                {
                    node->left = node_par->right->left;
                    node->right = node_par->right->right;
                }
                node_par->right = node;
                break;
        }
