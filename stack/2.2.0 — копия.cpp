...
bool StackOk(const Stack *st)
{
    return (st != NULL)
        ...
        NOT_BAG(&& (st->data[st->capacity] == BIRD) && (st->data[-1] == BIRD));     ///ÏĞÎÂÅĞÊÀ (ST->CAPACITY > 0) ËÈØÍßß
...
int StackDump(const Stack *st, const char *out_file_name, int pr)
{
    FILE *out_f = fopen(out_file_name, "a"); //!!À ÅÑËÈ ÔÀÉË ÍÅ ÓÄÀÑÒÑß ÑÎÇÄÀÒÜ - fixed
    assert(out_f != NULL);
...
{
    ST_ASSERT(st);
    NOT_BAG(st->data = (st_t*)realloc(st->data - 1, (st->capacity * 2 + 2) * sizeof(st_t)) + 1);  //REALLOC ÒÎÆÅ ÌÎÆÅÒ ÂÅĞÍÓÒÜ NULL - fixed in StackOk
    st->capacity *= 2;
    NOT_BAG(st->data[st->capacity] = BIRD);
    ST_ASSERT(st);
    return 0;
